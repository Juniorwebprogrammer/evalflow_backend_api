...
public class GetSubmissionByIdEndpointTests : IntegrationTestBase
{
    [Fact]
    public async Task Get_SubmissionById_WithoutJwt_ReturnsUnauthorized()
    {
        var response = await Client.GetAsync("/evaluation-submissions/1");

        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }

    [Fact]
    public async Task Get_SubmissionById_WithoutApiKey_ReturnsUnauthorized()
    {
    ...
    [Fact]
    public async Task Get_SubmissionById_BelongingToAnotherRespondent_ReturnsNotFound()
    {
        ...
        var response = await client.GetAsync($"/evaluation-submissions/{submissionId}");

        response.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }

    [Fact]
    public async Task Get_SubmissionById_AsRespondent_ReturnsSubmissionDetail()
    {
        var company = TestDataFactory.CreateCompany(identificationId: "tenant-1");
        var respondent = TestDataFactory.CreateUser(company, email: "respondent@example.com");
        var evaluated = TestDataFactory.CreateUser(company, email: "evaluated@example.com");
        ...
        var payload = await response.Content.ReadFromJsonAsync<SubmissionDetailDto>();
        payload!.SubmissionId.Should().Be(submissionId);
        payload.TemplateTitle.Should().Be("Template");
    }
}
